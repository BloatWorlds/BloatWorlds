...
void CMenus::RenderMenubar(CUIRect Rect)
{
	...
	{
		float Spacing = 3.0f;
		float NumSettingsButtons = 7.f;
		float ButtonWidth = (Box.w/NumSettingsButtons)-(Spacing*5.0)/NumSettingsButtons;
		...
		Box.VSplitLeft(Spacing, 0, &Box); // little space
		Box.VSplitLeft(ButtonWidth, &Button, &Box);
		static CButtonContainer s_BloatWorldsButton;
		if(DoButton_MenuTabTop(&s_BloatWorldsButton, Localize("Bloat"), Client()->State() == IClient::STATE_OFFLINE && Config()->m_UiSettingsPage==SETTINGS_BLOATWORLDS, &Button,
			Config()->m_UiSettingsPage == SETTINGS_BLOATWORLDS ? 1.0f : NotActiveAlpha, 1.0f, Corners))
		{
			m_pClient->m_pCamera->ChangePosition(CCamera::POS_SETTINGS_BLOATWORLDS);
			Config()->m_UiSettingsPage = SETTINGS_BLOATWORLDS;
		}
	}
...
void CMenus::RenderLoading(int WorkedAmount)
{
	...
	TextRender()->TextOutlineColor(0.0f, 0.0f, 0.0f, 0.3f);
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
	UI()->DoLabel(&Rect, "Bloatworlds", 48.0f, CUI::ALIGN_CENTER);
